...
const [priceRange, setPriceRange] = useState({
    ...
    try {
      ...
      if (!search.value) {
        response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/attractions/all?limit=${limit}
          &skip=${skip}&ratings=[${ratingsArray.join(',')}]&categories=${JSON.stringify(filters.category)}&priceFrom=${JSON.stringify(filters.priceFrom)}&priceTo=${JSON.stringify(filters.priceTo)}`,
          { next: { revalidate: 10 } }
        );
      } else {
        ...
        response = await fetch(
          ...
          &ratings=[${ratingsArray.join(',')}]&categories=${JSON.stringify(filters.category)}&priceFrom=${JSON.stringify(filters.priceFrom)}&priceTo=${JSON.stringify(filters.priceTo)}`
        );
      }
  ...
  async function setSearchAttractions(
    ...
    async function fetchAttractons() {
      if(typeof(destination)==="string"){
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/attractions/all?limit=${limit}&skip=${skip}&destination=${destination}&ratings=[${ratingsArray.join(',')}]&categories=${JSON.stringify(filters.category)}`
        );
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/attractions/all?limit=${limit}&skip=${skip}&destination=${destination?.name}&ratings=[${ratingsArray.join(',')}]&categories=${JSON.stringify(filters.category)}`
        );
    ...
    if (responseData?.attractions?.data?.length > 0) {
      setSearch({ destination: responseData?.attractions?.data[0]?.destination?.name as string, value: true });
  ...
  useEffect(() => {
    ...
    async function getActionAll() {
      try {
        const   response = await fetch(
             `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/attractions/all?limit=${10000}&skip=${0}`,
             { next: { revalidate: 10 } }
           );
           return response.json()
      } catch (error) {
    ...
    const prices = response?.attractions?.data?.map((elem:any) => elem.activity.lowPrice);

    // Filter out undefined or null values from the prices array
    const filteredPrices = prices.filter((price:any) => typeof price === 'number');
  ...
  return (
    <main>
      <SearchContainer
       setSearchAttractions={setSearchAttractions} />
       
      <div className="my-16 padding flex flex-col gap-5">
     ...
     sliderValue={sliderValue}
      priceRange={priceRange} ratingsArray={ratingsArray} setRatingsArray={setRatingsArray} setSkip={setSkip} setFilters={setFilters} filters={filters}/>
        ...
        <section className="w-full  md:w-4/6 lg:w-5/6 ">
          <section className="w-full  flex flex-col sm:flex-row justify-between ">
            <div className="text-2xl">
              ...
              {(allAttraction?.length <= 0 ||!allAttraction?.length  )&& `No Attractions Found`}
            </div>
