...
export default function FavouriteCard({attraction}:AttractionCardProps) {
  ...
  return (
    <div  className='flex flex-col sm:flex-row h-fit  border mb-2'>
...
onClick={handleLikeExc}
 className='absolute right-5 top-5 text-white cursor-pointer'/>
    ...
    <img className='w-full h-full' src={process.env.NEXT_PUBLIC_SERVER_URL+attraction?.images[0]} alt="img" />
    </div>
  <div className='p-2 w-full h-[150px] flex flex-col justify-around pt-5'>
    ...
    <div className='text-sm  text-red-600'>{capitalizeFirstLetters(attraction?.category?.categoryName)}</div>
        ...
        <div className='text-stone-500'> <span className='text-black'>{attraction?.activity?.lowPrice } AED</span>  /person</div>
        

         <div className='flex items-center text-sm'>
        <FaStar className='w-4 h-4 text-yellow-400' />
        <div>{formatNumberToOneDecimalPlace(attraction?.averageRating)}</div>
        </div>
         </div>
  </div>
</div>
